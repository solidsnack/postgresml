...
pub fn smartcore_train(
    ...
    let json_value = serde_json::to_value(hyperparams)?;
    pgx::info!("parsing hyperparams: {:?}", json_value);
    match algorithm {
        Algorithm::linear => match task {
            Task::regression => {
                let hyperparams: smartcore::linear::linear_regression::LinearRegressionParameters = serde_json::from_value(json_value).with_context(||format!("Could not parse hyperparams: {:?}", hyperparams))?;
                pgx::info!("fitting task: {:?}, algorithm: {:?}, hyperparams: {:?}", task, algorithm, hyperparams);
                Ok(Box::new(
            ...
            Task::classification => {
                let hyperparams: smartcore::linear::logistic_regression::LogisticRegressionParameters<f32> = serde_json::from_value(json_value).with_context(||format!("Could not parse hyperparams: {:?}", hyperparams))?;
                pgx::info!("fitting task: {:?}, algorithm: {:?}, hyperparams: {:?}", task, algorithm, hyperparams);
                Ok(Box::new(
        ...
        Algorithm::elastic_net => match task {
            Task::regression => {
                let hyperparams: smartcore::linear::elastic_net::ElasticNetParameters<f32> = serde_json::from_value(json_value).with_context(||format!("Could not parse hyperparams: {:?}", hyperparams))?;
                pgx::info!("fitting task: {:?}, algorithm: {:?}, hyperparams: {:?}", task, algorithm, hyperparams);
                Ok(Box::new(
            ...
            Task::classification => bail!("{:?} does not support {:?}", algorithm, task),
        },
        Algorithm::lasso => match task {
            Task::regression => {
                let hyperparams: smartcore::linear::lasso::LassoParameters<f32> = serde_json::from_value(json_value).with_context(||format!("Could not parse hyperparams: {:?}", hyperparams))?;
                pgx::info!("fitting task: {:?}, algorithm: {:?}, hyperparams: {:?}", task, algorithm, hyperparams);
                Ok(Box::new(
        ...
        Algorithm::ridge => match task {
            Task::regression => {
                let hyperparams: smartcore::linear::ridge_regression::RidgeRegressionParameters<f32> = serde_json::from_value(json_value).with_context(||format!("Could not parse hyperparams: {:?}", hyperparams))?;
                pgx::info!("fitting task: {:?}, algorithm: {:?}, hyperparams: {:?}", task, algorithm, hyperparams);
                Ok(Box::new(
        ...
        Algorithm::random_forest => match task {
            Task::regression => {
                let hyperparams: smartcore::ensemble::random_forest_regressor::RandomForestRegressorParameters = serde_json::from_value(json_value).with_context(||format!("Could not parse hyperparams: {:?}", hyperparams))?;
                pgx::info!("fitting task: {:?}, algorithm: {:?}, hyperparams: {:?}", task, algorithm, hyperparams);
                Ok(Box::new(
            ...
            Task::classification => {
                let hyperparams: smartcore::ensemble::random_forest_classifier::RandomForestClassifierParameters = serde_json::from_value(json_value).with_context(||format!("Could not parse hyperparams: {:?}", hyperparams))?;
                pgx::info!("fitting task: {:?}, algorithm: {:?}, hyperparams: {:?}", task, algorithm, hyperparams);
                Ok(Box::new(
        ...
        // Algorithm::knn => match task {
        //     Task::regression => {
        //         let hyperparams: smartcore::neighbors::knn_regressor::KNNRegressorParameters<f32, smartcore::math::distance::euclidian::Euclidian> = serde_json::from_value(json_value).with_context(||format!("Could not parse hyperparams: {:?}", hyperparams))?;
        //         pgx::info!("fitting task: {:?}, algorithm: {:?}, hyperparams: {:?}", task, algorithm, hyperparams);
        //         Ok(Box::new(
        ...
        //     }
        //     Task::classification => {
        //         let hyperparams: smartcore::neighbors::knn_classifier::KNNClassifierParameters<f32, smartcore::math::distance::euclidian::Euclidian> = serde_json::from_value(json_value).with_context(||format!("Could not parse hyperparams: {:?}", hyperparams))?;
        //         pgx::info!("fitting task: {:?}, algorithm: {:?}, hyperparams: {:?}", task, algorithm, hyperparams);
        //         Ok(Box::new(
        ...
        Algorithm::bernoulli_naive_bayes => match task {
            Task::regression => bail!("{:?} does not support {:?}", algorithm, task),
            Task::classification => {
                let hyperparams: smartcore::naive_bayes::bernoulli::BernoulliNBParameters<f32> = serde_json::from_value(json_value).with_context(||format!("Could not parse hyperparams: {:?}", hyperparams))?;
                pgx::info!("fitting task: {:?}, algorithm: {:?}, hyperparams: {:?}", task, algorithm, hyperparams);
                Ok(Box::new(
                    smartcore::naive_bayes::bernoulli::BernoulliNB::fit(
                        &x_train, &y_train, hyperparams,
                    )?,
                ))
            }
        ...
        Algorithm::gaussian_naive_bayes => match task {
            Task::regression => bail!("{:?} does not support {:?}", algorithm, task),
            Task::classification => {
                let hyperparams: smartcore::naive_bayes::gaussian::GaussianNBParameters<f32> = serde_json::from_value(json_value).with_context(||format!("Could not parse hyperparams: {:?}", hyperparams))?;
                pgx::info!("fitting task: {:?}, algorithm: {:?}, hyperparams: {:?}", task, algorithm, hyperparams);
                Ok(Box::new(
                    smartcore::naive_bayes::gaussian::GaussianNB::fit(
                        &x_train, &y_train, hyperparams,
                    )?,
                ))
            }
        ...
        Algorithm::categorical_naive_bayes => match task {
            Task::regression => bail!("{:?} does not support {:?}", algorithm, task),
            Task::classification => {
                let hyperparams: smartcore::naive_bayes::categorical::CategoricalNBParameters<f32> = serde_json::from_value(json_value).with_context(||format!("Could not parse hyperparams: {:?}", hyperparams))?;
                pgx::info!("fitting task: {:?}, algorithm: {:?}, hyperparams: {:?}", task, algorithm, hyperparams);
                Ok(Box::new(
                    smartcore::naive_bayes::categorical::CategoricalNB::fit(
                        &x_train, &y_train, hyperparams,
                    )?,
                ))
            }
        ...
        Algorithm::multinomial_naive_bayes => match task {
            Task::regression => bail!("{:?} does not support {:?}", algorithm, task),
            Task::classification => {
                let hyperparams: smartcore::naive_bayes::multinomial::MultinomialNBParameters<f32> = serde_json::from_value(json_value).with_context(||format!("Could not parse hyperparams: {:?}", hyperparams))?;
                pgx::info!("fitting task: {:?}, algorithm: {:?}, hyperparams: {:?}", task, algorithm, hyperparams);
                Ok(Box::new(
                    smartcore::naive_bayes::multinomial::MultinomialNB::fit(
                        &x_train, &y_train, hyperparams,
                    )?,
                ))
            }
        ...
        _ => {bail!("SmartCore does not support algorithm: `{:?}`", algorithm)}
    }
        ...
        //             let params: smartcore::linear::lasso::Lasso<f32> = serde_json::from_value(serde_json::to_value(hyperparams).unwrap()).unwrap();
        //             Box::new(
        //                 smartcore::linear::lasso::Lasso::fit(
